...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, getMonth, getYear, isToday } from 'date-fns';
import { CheckCircle, XCircle, Clock, Play, Pause, ChevronRight } from 'lucide-react';
import {
  BarChart,
  Bar,
  XAxis,
...
const generateAttendanceData = (month: number, year: number) => {
  ...
  return days.map(day => {
    ...
    if (status !== 'Weekend') {
      const checkInHour = status === 'Present' ? 8 + Math.floor(Math.random() * 2) : 12 + Math.floor(Math.random() * 2);
      checkIn = new Date(day);
      checkIn.setHours(checkInHour, 15 + Math.floor(Math.random() * 30), 0);

      if (status === 'Present') {
        checkOut = new Date(day);
        checkOut.setHours(17 + Math.floor(Math.random() * 2), 30 + Math.floor(Math.random() * 30), 0);
      } else if (status === 'Half-Day') {
        checkOut = new Date(day);
        checkOut.setHours(13 + Math.floor(Math.random() * 2), 15 + Math.floor(Math.random() * 30), 0);
      }
    }
...
export function Attendance() {
  ...
  const [todayCheckIn, setTodayCheckIn] = useState<Date | null>(new Date(Date.now() - 3600000 * 8));
  ...
  const attendanceData = useMemo(() => {
    return baseAttendanceData.map(record => {
      if (isToday(record.date)) {
        return {
          ...record,
          status: todayCheckIn && !todayCheckOut ? 'Present' : (todayCheckIn && todayCheckOut ? 'Present' : record.status),
  ...
  // Calculate stats
  const stats = useMemo(() => {
    let present = 0, absent = 0, halfDay = 0, weekends = 0, lateArrivals = 0;
    let totalMinutes = 0, workingDays = 0;
    ...
    const avgHours = workingDays > 0 ? Math.round((totalMinutes / workingDays) / 60 * 10) / 10 : 0;

    return { present, absent, halfDay, weekends, lateArrivals, avgHours, workingDays };
  }, [attendanceData]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Track your daily attendance, working hours, and history.</p>
        </div>
            ...
            <CardContent className="p-8 bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
              <div className="text-center space-y-6">
                <div className="space-y-2">
                  <p className="text-slate-600 dark:text-slate-300 font-semibold text-lg">Current Time</p>
              ...
              <CardContent className="p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-base">Total Present</p>
                    <p className="text-3xl font-bold text-slate-900 dark:text-white mt-0.5">
                      {stats.present}
                    </p>
                    <p className="text-slate-400 text-sm mt-0.5">days this month</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-none shadow-md hover:shadow-lg transition-all overflow-hidden">
              <CardContent className="p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-base">Total Absent</p>
                    <p className="text-3xl font-bold text-slate-900 dark:text-white mt-0.5">
                      {stats.absent}
                    </p>
                    <p className="text-slate-400 text-sm mt-0.5">days this month</p>
            ...
            <Card className="border-none shadow-md hover:shadow-lg transition-all overflow-hidden">
              <CardContent className="p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-base">Late Arrivals</p>
                    <p className="text-3xl font-bold text-slate-900 dark:text-white mt-0.5">
                      {stats.lateArrivals}
                    </p>
                    <p className="text-slate-400 text-sm mt-0.5">days this month</p>
            ...
            <Card className="border-none shadow-md hover:shadow-lg transition-all overflow-hidden">
              <CardContent className="p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-base">Avg. Working Hours</p>
                    <p className="text-3xl font-bold text-slate-900 dark:text-white mt-0.5">
                      {stats.avgHours}h
                    </p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  <th className="text-left pb-4 pl-4 pr-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Date</th>
                  <th className="text-left pb-4 pl-4 pr-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Status</th>
                  <th className="text-left pb-4 pl-4 pr-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Check In</th>
                  <th className="text-left pb-4 pl-4 pr-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Check Out</th>
                  <th className="text-right pb-4 pl-4 pr-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Duration</th>
                </tr>
              </thead>
              <tbody>
                {[...attendanceData].reverse().filter(d => d.status !== 'Weekend').slice(0, 5).map((record, index) => (
                  <tr key={index} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors">
                    <td className="py-4 pl-4 pr-4 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-gray-900 dark:text-white">{format(record.date, 'EEEE')}</span>
                        <span className="text-gray-500 dark:text-gray-400">{format(record.date, 'MMM d, yyyy')}</span>
                      </div>
                    </td>
                    <td className="py-4 pl-4 pr-4">
                      <div className="flex items-center gap-2">
                        ...
                        {record.status === 'Half-Day' && <Clock className="w-4 h-4 text-amber-500" />}
                        <Badge variant={
